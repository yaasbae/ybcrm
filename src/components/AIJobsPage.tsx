...
const TYPE_LABELS: Record<string, string> = {
  ...
  'business.daily_digest': 'Ежедневная сводка бизнеса',
...
function jobPreview(job: Job) {
  if (job.type === 'business.daily_digest') return 'Read-only проверка продаж, оплат, заказов, склада, производства и финансов.';
...
export default function AIJobsPage() {
  ...
  const runDailyDigest = async () => {
    ...
    try {
      ...
      await post('/api/ai-jobs', { type: 'business.daily_digest', idempotencyKey: `daily-digest-${date}`, payload: { date, source: 'owner-dashboard' } });
      if (enabled) await post('/api/ai-jobs/run');
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Сводка не сформирована'); }
    finally { setActionLoading(false); }
  };
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => void runDailyDigest()} disabled={actionLoading} className="flex min-h-11 items-center gap-2 rounded-lg bg-[#7D7DE6] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#6f6fd8] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#7D7DE6] disabled:opacity-50"><History size={16}/> {actionLoading ? 'Формируем…' : 'Сводка за сегодня'}</button>
