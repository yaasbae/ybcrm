...
export const ProductionPage: React.FC<ProductionPageProps> = ({
  ...
  const addEntry = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const createdEntry = await addDoc(collection(db, 'production_entries'), {
      ...
      void emitPushEvent('production_changed', `production-created:${createdEntry.id}`, { action: 'Добавлено', productName: normalizedProductName, quantity: parsedQuantity });
      resetForm();
  ...
  const saveEdit = async (entryId: string) => {
    ...
    try {
      ...
      void emitPushEvent('production_changed', `production-updated:${entryId}:${Date.now()}`, { action: 'Изменено', productName: editDraft.productName, quantity: parsedQuantity });
      cancelEdit();
  ...
  const removeEntry = async (entry: ProductionEntry) => {
    ...
    try {
      ...
      void emitPushEvent('production_changed', `production-deleted:${entry.id}:${Date.now()}`, { action: 'Удалено', productName: entry.productName, quantity: entry.quantity });
