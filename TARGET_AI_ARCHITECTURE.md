...
## Первый ограниченный диспетчер

Реализован безопасный pilot для Telegram-обращений менеджеров. Он работает только с коллекцией `ai_incidents`, ставит классификацию в общую job queue, маскирует базовые персональные данные перед AI и не имеет write-tools к заказам, клиентам, финансам, производству или коду. Любое будущее исправление создаётся отдельным действием и требует approval.
