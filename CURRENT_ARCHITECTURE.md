...
Полноценной job queue нет. Используются `setTimeout`, `setInterval`, in-memory `Map/Set`, polling из браузера и коллекция `push_jobs`. Это означает:

- процесс теряет незавершённую работу при рестарте Cloud Run;
- часть сверки банка и CDEK зависит от открытого браузера;
- нет общего retry policy, idempotency key и dead-letter queue;
